...
public final class MessageBodyBuilder {
	...
	private static String getOriginal(MessageBean msgBean, boolean isHtml) {
		...
		if (CarrierCode.WEBMAIL.value().equalsIgnoreCase(msgBean.getOriginalMail().getCarrierCode())) {
	...
	private static String getEmailIdSection(MessageBean msgBean, boolean isHtml) {
		String section = "";
		if (!CarrierCode.WEBMAIL.value().equalsIgnoreCase(msgBean.getCarrierCode()) && msgBean.getMsgId() != null) {
			if (isHtml) {
